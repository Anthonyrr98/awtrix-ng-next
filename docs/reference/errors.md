...
| `wrongChip` | 400 | `POST /update` (firmware upload) | The upload is not an update image for this device: built for a different chip, for the other kind of ESP32-S3 PSRAM (quad vs octal), a `usb-*.bin` meant for a first flash over USB, not a firmware image, or its SHA-256 differs from the release manifest. `message` says which. The running firmware is untouched. |
| `updateFailed` | 500 | `POST /update` (firmware upload) | The OTA flash writer rejected the image or had insufficient usable storage. `message` contains its lower-level reason. |
